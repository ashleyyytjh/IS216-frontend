...
export default function FilterBar() {
  ...
  return (
    <section className="mx-auto max-w-6xl flex gap-4 text-sm">
      <div className="relative flex-1 opacity-60 focus-within:opacity-100 transition-opacity">
          ...
          >
            {options.map((option) => (
              <RadioGroupPrimitive.Item
                key={option.value}
                value={option.value}
                className="ring-[1px] ring-border rounded-md py-1 px-3 data-[state=checked]:bg-muted"
              >
